...
export default function InviteModal({
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    // Insert participant directly using email (no account required)
    ...
    if (insertError) {
      ...
      setSuccess(`Invited ${email}! They can join using the link.`);
      setEmail("");
      onInvited();
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      <div className="relative bg-cream rounded-2xl shadow-2xl w-full max-w-md p-6 animate-slide-up">
        ...
        <form onSubmit={handleInvite} className="space-y-3">
          ...
          {success && (
            <p className="text-sage text-xs flex items-center gap-1.5">
        ...
        {participants.length > 0 && (
          <div className="mt-5 pt-4 border-t border-stone/15">
            <p className="text-xs font-semibold text-stone uppercase tracking-wide mb-3">
              Current participants
            </p>
            <div className="space-y-2">
              {participants.map((p) => (
                <div key={p.id} className="flex items-center gap-2">
                  <div
                    className={`w-1.5 h-1.5 rounded-full ${p.status === "joined" ? "bg-sage" : "bg-stone/40"}`}
                  />
